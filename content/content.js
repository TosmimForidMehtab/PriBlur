...
if (typeof window.privacyBlurManager === 'undefined') {
    window.privacyBlurManager = (function() {
        ...
        // --- EVENT LISTENERS (REWRITTEN FOR SCROLLING) ---
        ...
        function setInteractionState(state) {
            ...
            if (state === 'draw') {
                configOverlay.style.pointerEvents = 'auto'; configOverlay.style.cursor = 'crosshair';
                blurElements.forEach(el => el.style.pointerEvents = 'none');
            } else {
                configOverlay.style.pointerEvents = 'none';
                blurElements.forEach(el => { el.style.pointerEvents = 'auto'; el.style.cursor = drawMode === 'eraser' ? 'cell' : 'move'; });
            }
        }
        ...
        function addToolbarListeners() {
            ...
            const setActiveTool = (tool) => {
                ...
                if (tool === 'eraser') setInteractionState('drag/erase'); else setInteractionState('draw');
            };
            addBtn.onclick = () => setActiveTool('rect'); rectBtn.onclick = () => setActiveTool('rect'); circleBtn.onclick = () => setActiveTool('circle');
            drawBtn.onclick = () => setActiveTool('custom'); eraserBtn.onclick = () => setActiveTool('eraser');
        ...
        // --- INITIAL LOAD (REVISED AND MORE ROBUST) ---
        (async () => {
            ...
            if (isEnabled) {
                try {
                    // This is the defensive part. We check if the response exists before destructuring.
                    const response = await chrome.runtime.sendMessage({ type: 'GET_BLURS' });
                    const blurs = response ? response.blurs : []; // If response is undefined, default to an empty array.

                    if (blurs && blurs.length > 0) {
                        applyAllBlurs(blurs);
                    }
                } catch (e) {
                    // This catch block will now mostly handle cases where the extension context is invalidated (e.g., during an update).
                    console.warn("PrivacyBlur: Could not communicate with the background script on initial load. This can happen during an extension reload and is usually not a problem.");
                }
            }
        })();
